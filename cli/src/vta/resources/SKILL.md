...
操作后立即 `vta state` 检查 `newState` 确认变化。**每一步都要确认。**
...
5. 用 `vta tap <x> <y>` 坐标盲点兜底

### 第四步：等待异步响应

触发网络请求或 AI 回复后，**不要假设立即生效**。使用 `vta watch --until` 条件轮询：

```bash
# 等推荐卡出现 + AI 回复完成
vta -p com.xt.retouch watch -t 120 -i 1000 --until \
  "'image_list' in str(s) and '收到' not in str(s) and '预计还需' not in str(s)"
```
...
3. **尝试备选路径** — 换一个 id、换文字匹配、换 class name、换坐标定位
...
6. 以上都失败 → `vta state` 获取 bounds，计算中心坐标，`vta tap <x> <y>`
